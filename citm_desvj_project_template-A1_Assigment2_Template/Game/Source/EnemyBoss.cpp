...
bool EnemyBoss::Update(float dt)
{
	...
	if (currentAnimation == &attack1Anim && currentAnimation->HasFinished()) {
		attack = false;
		attack1Anim.Reset();
	...
	if (attack && currentAnimation == &attack1Anim && currentAnimation->GetCurrentFrameCount() >= 4 && !attackBody)
	{
		if (right)
		{
	...
	if (attack && currentAnimation == &attack1Anim && currentAnimation->GetCurrentFrameCount() >= 6.8 && attackBody)
	{
		attack = false;
		attackBody = false;
		if (damage)
